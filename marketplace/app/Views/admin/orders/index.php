...
$hasFilter = $q !== '' || $status !== '' || $kind !== '' || $open || $payment !== '';

/** Orders list URL with the current filters, overriding some of them. */
$link = static function (array $over = []) use ($status, $q, $kind, $open, $payment): string {
    $params = array_filter(['status' => $status, 'kind' => $kind, 'q' => $q, 'open' => $open ? '1' : '', 'payment' => $payment] + [], static fn ($v) => $v !== '' && $v !== null);
    foreach ($over as $k => $v) {
        if ($v === '' || $v === null) {
            unset($params[$k]);
        } else {
...
<div class="tabs">
    <a href="<?= e($link(['status' => '', 'open' => '', 'payment' => ''])) ?>" class="<?= $status === '' && !$open && $payment === '' ? 'active' : '' ?>">All <span class="tab-count"><?= (int) $allCount ?></span></a>
    <?php foreach (OrderController::STATUSES as $st): ?>
        <a href="<?= e($link(['status' => $st, 'open' => '', 'payment' => ''])) ?>" class="<?= $status === $st ? 'active' : '' ?>"><?= e(Forms::label($st)) ?> <span class="tab-count"><?= (int) ($counts[$st] ?? 0) ?></span></a>
    <?php endforeach; ?>
    <a href="<?= e($link(['status' => '', 'open' => '', 'payment' => 'awaiting'])) ?>" class="<?= $payment === 'awaiting' ? 'active' : '' ?>">Awaiting payment <span class="tab-count<?= $awaitingCount ? ' hot' : '' ?>"><?= (int) $awaitingCount ?></span></a>
</div>
...
<form method="get" action="<?= e(url('/admin/orders')) ?>" class="card filters">
    ...
    <?php if ($payment !== ''): ?><input type="hidden" name="payment" value="<?= e($payment) ?>"><?php endif; ?>
...
<?php if ($payment === 'awaiting'): ?>
    <p class="muted">Showing prepaid orders (remote zones and digital items) still waiting for the customer's OMT/Whish payment. Open one and press &quot;Mark payment received&quot; when it arrives.</p>
<?php endif; ?>
<?php if ($open): ?>
...
<section class="card">
    ...
    <?php else: ?>
        <div class="table-wrap">
            <table class="data">
                <thead><tr><th>Order</th><th>Placed</th><th>Buyer</th><th>Phone</th><th class="num">Items</th><th class="num">Total</th><th class="num">Credit used</th><th class="num">Cash to collect</th><th>Status</th><th>Payment</th><th></th></tr></thead>
                <tbody>
                <?php foreach ($orders as $o): $sp = $o['split']; ?>
                    <tr>
                        ...
                        <td class="num">
                            ...
                            <?php elseif ($sp['kind'] === 'digital' || !empty($sp['all_prepaid'])): ?>
                                <span class="prepaid-note" title="Prepaid via OMT/Whish, nothing to collect at the door">PREPAID</span>
                            <?php else: ?>
                        ...
                        <td><?= Forms::paymentPill($o['payment_status']) ?></td>
