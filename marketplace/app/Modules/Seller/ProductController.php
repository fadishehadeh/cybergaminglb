...
/**
 * A seller's own listings. Every query carries "seller_id = <logged-in seller>"; another seller's id is a 404.
 * Sellers can never set a listing live (status 'active') or touch commission: the server decides both.
 */
final class ProductController extends PortalController
{
    public const STATUSES = ['pending', 'active', 'hidden', 'sold'];

    public function index(Request $request): void
    {
        $sid    = (int) $this->seller()['id'];
        ...
        $this->view('products/index', compact('products', 'pager', 'status', 'counts', 'kinds'));
    }
    ...
    public function withdraw(Request $request, string $id): void
    {
        ...
        if ($changed) {
            $this->ok('"' . $product['title'] . '" was withdrawn from the shop. Relist it any time.');
        } else {
    ...
    public function relist(Request $request, string $id): void
    {
        ...
        if ((int) $product['stock'] < 1) {
            $this->back('/seller/products/' . $product['id'] . '/edit', 'Set the stock above 0 before relisting "' . $product['title'] . '".');
        }
        $changed = db()->execute(
    ...
    public function delete(Request $request, string $id): void
    {
        ...
        if ((int) db()->fetchValue('SELECT COUNT(*) FROM order_items WHERE product_id = ?', [$product['id']]) > 0) {
            $this->back('/seller/products', '"' . $product['title'] . '" has been ordered before, so it cannot be deleted. You can withdraw it instead.');
        }
        $deleted = db()->transaction(static function () use ($product): int {
    ...
    /** The seller's OWN product or a 404: this is the IDOR guard for every action above. */
    private function find(int $id): array
    {
        $product = db()->fetch('SELECT * FROM products WHERE id = ? AND seller_id = ?', [$id, $this->seller()['id']]);
        if ($product === null) {
            Response::abort(404);
        }
        return $product;
    ...
    private function save(Request $request, ?array $product): void
    {
        $seller = $this->seller();
        $isEdit = $product !== null;
        $back   = $isEdit ? '/seller/products/' . $product['id'] . '/edit' : '/seller/products/new';
        $errors = [];
        ...
        // Anti-bypass: buyers only ever talk to CyberGaming.
        ...
        if (!$isEdit) {
            ...
            // Anything a buyer can see (everything except stock) needs a fresh look from us before it stays live.
            ...
            if ($status === 'active' && $contentChanged) {
                ...
                $note   = ' It was sold out, so it is waiting for approval again.';
        ...
        $msg = '"' . $title . '" saved. Buyers will see ' . money($price) . ' (you receive ' . money($priceIn) . ').';
        if (!$isEdit) {
        ...
        $this->ok($msg);
        $this->redirect($isEdit ? '/seller/products/' . $id . '/edit' : '/seller/products');
    }
